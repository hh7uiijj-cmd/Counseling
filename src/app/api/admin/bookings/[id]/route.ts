...
import { notifyBookingStatus } from "@/lib/bookingActions";
...
export async function PATCH(
  ...
  if (!newSlotId) {
    ...
    if (status) await notifyBookingStatus(booking, status);
  ...
  if (status) await notifyBookingStatus(result.booking, status);
